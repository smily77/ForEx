// ForEx_2_UART_Frankfurter – GSM/LTE Version mit frankfurter.app API
...
//   - BMP180 entfernt (Hardware-UART0 jetzt für LTE-UART belegt)
...
//   GPIO 3 / RX0 (ESP8266) → TX-Pin des Moduls   (ESP8266 liest)
//   GPIO 1 / TX0 (ESP8266) → RX-Pin des Moduls   (ESP8266 schreibt)
//   Baudrate fix: 19200
// ============================================================
#define LTE_RX_PIN   3      // ESP8266 RX0 (GPIO3) ← Modul TX
#define LTE_TX_PIN   1      // ESP8266 TX0 (GPIO1) → Modul RX
#define LTE_BAUD  19200

#define lteSerial Serial
...
void setup() {
  ...
  // Spart ~20 KB Heap; LTE läuft über Hardware-UART.
  WiFi.mode(WIFI_OFF);
  WiFi.forceSleepBegin();
  delay(1);  // forceSleep braucht 1 Zyklus

  Serial.begin(LTE_BAUD);
...
#endif
      // Ticker pausieren: während TLS übernehmen die internen Timeouts die Absicherung.
      secondTick.detach();
