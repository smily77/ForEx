# ForEx_2_UART_Frankfurter – Schaltungsschema

Hardware-Version: **ForEx_1_1** mit BK-7670 LTE-Modul (AND Global) anstelle des BMP180.
Hardware-UART0 (GPIO3/GPIO1) ist für die UART-Verbindung zum LTE-Modul reserviert.
...
graph TD
    subgraph ESP["ESP8266 (NodeMCU / WeMos D1 Mini)"]
        G0["GPIO 0  – LED PWM"]
        G2["GPIO 2  – TFT_DC"]
        G3["GPIO 3  – LTE RX0 ← Modul TX"]
        G1["GPIO 1  – LTE TX0 → Modul RX"]
        G12["GPIO 12 – TFT_RST"]
...
```mermaid
graph LR
    ESP["ESP8266"] -->|GPIO 1 / TX0  →  RX| MOD["BK-7670"]
    MOD            -->|TX  →  GPIO 3 / RX0| ESP
    ESP            -->|3.3 V + GND      | MOD

    note1["Baudrate: 19200 (fix im Modul)"]
    note2["Hardware UART0\nRX=GPIO3, TX=GPIO1\nSerial exklusiv fuer Modem"]
```
...
| 3    | **Eingang** | UART RX0 (← Modul TX)    | BK-7670 TX                    |
| 1    | **Ausgang** | UART TX0 (→ Modul RX)    | BK-7670 RX                    |
...
Die APN-Einstellung wird als Konstante in `ForEx_2_UART_Frankfurter.ino` gewählt:
...
> `ForEx_2_UART_Frankfurter.ino` anpassen.
